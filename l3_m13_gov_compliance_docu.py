...
if __name__ == "__main__":
    ...
    print("Module 13.2: Governance & Compliance Documentation")
    print("=" * 60)

    if len(sys.argv) < 2:
        print("\nUsage:")
        print("  python l3_m13_gov_compliance_docu.py privacy     # Generate privacy policy")
        print("  python l3_m13_gov_compliance_docu.py soc2        # Generate SOC 2 docs")
        print("  python l3_m13_gov_compliance_docu.py incident    # Generate incident playbook")
        print("  python l3_m13_gov_compliance_docu.py evidence    # Collect evidence")
        print("  python l3_m13_gov_compliance_docu.py all         # Generate all documents")
        sys.exit(1)
    ...
    if command == "privacy":
        ...
        Path(output_path).write_text(policy)
        print(f"\n✅ Privacy policy generated: {output_path}")
        print(f"   Length: {len(policy)} characters")

    elif command == "soc2":
        doc = create_sample_soc2_controls()
        output_path = "soc2_controls.md"
        Path(output_path).write_text(doc)
        print(f"\n✅ SOC 2 controls generated: {output_path}")
        print(f"   Length: {len(doc)} characters")

    elif command == "incident":
        playbook = create_sample_incident_playbook()
        output_path = "incident_response_playbook.md"
        Path(output_path).write_text(playbook)
        print(f"\n✅ Incident response playbook generated: {output_path}")
        print(f"   Length: {len(playbook)} characters")

    elif command == "evidence":
        if not ComplianceConfig.has_aws_credentials():
            print("\n⚠️  Warning: AWS credentials not configured")
            print("   Generating mock evidence data only")

        evidence = run_evidence_collection()
        print(f"\n✅ Evidence collection complete")
        print(f"   Evidence types: {len(evidence)}")
        print(f"   Location: ./evidence/{datetime.now().strftime('%Y-%m')}/")

    elif command == "all":
        print("\nGenerating all compliance documents...\n")

        # Privacy policy
        policy = create_sample_privacy_policy()
        Path("privacy_policy.md").write_text(policy)
        print("✅ Privacy policy generated")

        # SOC 2 controls
        doc = create_sample_soc2_controls()
        Path("soc2_controls.md").write_text(doc)
        print("✅ SOC 2 controls generated")

        # Incident response
        playbook = create_sample_incident_playbook()
        Path("incident_response_playbook.md").write_text(playbook)
        print("✅ Incident response playbook generated")

        # Evidence
        if not ComplianceConfig.has_aws_credentials():
            print("⚠️  Skipping evidence collection (no AWS credentials)")
        else:
            evidence = run_evidence_collection()
            print("✅ Evidence collection complete")

        print(f"\n🎉 All compliance documentation generated!")
