...
if __name__ == "__main__":
    ...
    print("HyDE Module - CLI Examples\n" + "="*50)
    ...
    if not openai_key:
        print("⚠️  OPENAI_API_KEY not set. Skipping examples.")
        print("   Set your API key: export OPENAI_API_KEY='your-key'")
        exit(0)

    print("✓ OpenAI API key found")
    if not pinecone_key or not pinecone_index:
        print("⚠️  Pinecone credentials missing. Will skip vector search.")

    # Example 1: Basic hypothesis generation
    print("\n" + "="*50)
    print("Example 1: Hypothesis Generation")
    print("="*50)

    generator = HyDEGenerator(openai_api_key=openai_key)
    ...
    print("\n" + "="*50)
    print("Example 2: Query Classification")
    print("="*50)

    classifier = QueryClassifier()

    test_queries = [
        "What are the implications of equity compensation?",
        "When was the 2023 tax deadline?",
        "How does ISO taxation work?"
    ]

    for q in test_queries:
        classification = classifier.should_use_hyde(q)
        method = "HyDE" if classification["use_hyde"] else "Traditional"
        print(f"\nQuery: {q}")
